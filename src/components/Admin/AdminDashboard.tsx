...
export const AdminDashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState<"tournaments" | "users">(
    "tournaments"
  );
  ...
  const handleResetToLobby = async (tournamentId: string) => {
    if (
      !confirm(
        "Are you sure you want to reset this tournament to lobby? This will delete all matches."
      )
    ) {
      return;
    }
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to reset tournament"
      );
    } finally {
      setActionLoading(null);
    }
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <div className="flex items-center justify-end space-x-2">
                            ...
                            {tournament.status !== "lobby" && (
                              <button
                                onClick={() =>
                                  handleResetToLobby(tournament.id)
                                }
                                disabled={actionLoading === tournament.id}
