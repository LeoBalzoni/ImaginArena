...
export const MatchScreen: React.FC = () => {
  ...
  const determineMatchPhase = async () => {
    ...
    const isReadyForVoting = await MatchService.isMatchReadyForVoting(currentMatch.id)
    ...
    const isVotingComplete = await MatchService.isVotingComplete(currentMatch.id, participants.length)
  ...
  const isUserParticipant = () => {
    ...
    return currentMatch.player1_id === user.id || currentMatch.player2_id === user.id
  }
  ...
  const getPlayer1 = () => participants.find(p => p.id === currentMatch?.player1_id)
  const getPlayer2 = () => participants.find(p => p.id === currentMatch?.player2_id)
  const getWinner = () => currentMatch?.winner_id ? participants.find(p => p.id === currentMatch.winner_id) : null
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="text-center mb-8">
        ...
        {matchPhase === 'results' && winner && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
            ...
            <h2 className="text-xl font-bold text-gray-900">Winner: {winner.username}!</h2>
          </div>
        )}
      </div>
      ...
      <div className="card mb-8">
        <h3 className="text-lg font-semibold text-gray-900 mb-2">Creative Prompt</h3>
      ...
      {matchPhase === 'submission' && (
        <div className="space-y-8">
          <div className="text-center">
            ...
            <h2 className="text-xl font-bold text-gray-900 mb-2">Submission Phase</h2>
            <p className="text-gray-600">Players are creating and submitting their images</p>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="card">
              <h4 className="font-semibold text-gray-900 mb-4">{player1?.username}</h4>
              {player1Submission ? (
                <div className="text-center">
            ...
            <div className="card">
              <h4 className="font-semibold text-gray-900 mb-4">{player2?.username}</h4>
              {player2Submission ? (
                <div className="text-center">
                  <CheckCircle className="w-8 h-8 text-green-600 mx-auto mb-2" />
                  <p className="text-green-600 font-medium">Submitted!</p>
      ...
      {matchPhase === 'voting' && (
        <div className="space-y-8">
          <div className="text-center">
            ...
            <h2 className="text-xl font-bold text-gray-900 mb-2">Voting Phase</h2>
      ...
      {matchPhase === 'results' && (
        <div className="space-y-8">
          <div className="grid md:grid-cols-2 gap-6">
            <div className={`card ${winner?.id === player1?.id ? 'border-green-500 bg-green-50' : ''}`}>
              <div className="flex items-center justify-between mb-4">
                <h4 className="font-semibold text-gray-900">{player1?.username}</h4>
                {winner?.id === player1?.id && <Trophy className="w-5 h-5 text-yellow-600" />}
              </div>
              {player1Submission && (
                <div>
                  <img
            ...
            <div className={`card ${winner?.id === player2?.id ? 'border-green-500 bg-green-50' : ''}`}>
              <div className="flex items-center justify-between mb-4">
                <h4 className="font-semibold text-gray-900">{player2?.username}</h4>
                {winner?.id === player2?.id && <Trophy className="w-5 h-5 text-yellow-600" />}
              </div>
              {player2Submission && (
                <div>
                  <img
